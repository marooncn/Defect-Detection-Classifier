# https://www.kaggle.com/demoon/deep-forest-1-4-public-log-loss-with-1-5-data/notebook
...
def load_train():
    ...
    folders = ['defect', 'normal']
    for fld in folders:
        index = folders.index(fld)
        print('Load folder {} (Index: {})'.format(fld, index))
        path = os.path.join('../data/train', fld)
...
def load_test():
    ...
    for fld in folders:
        index = folders.index(fld)
        path = os.path.join('../data/test', fld)
        files = glob.glob(path)
        for fl in files:
            x = get_im_cv2(fl)
            X_test.append(x)
            X_test_id.append(index)
...
gcf = gcForest(shape_1X=[256, 256], window=[256,256], tolerance=0.0, min_samples=7)
